...
def reset_benchmark_metrics() -> dict[str, Any]:
    perf_metrics.reset()
    return {"status": "ok", "message": "benchmark metrics reset"}
...
def fetch_records(
    ...
    req_id = perf_metrics.new_request_id()
    perf_metrics.mark_started()
    t_req_start = time.perf_counter_ns()

    t_meta_start = time.perf_counter_ns()
    register = _get_register()
    perf_metrics.add_stage_ns(req_id, "metadata_lookup", t_meta_start, time.perf_counter_ns())
    ...
    t_db_start = time.perf_counter_ns()
    ...
    perf_metrics.add_stage_ns(req_id, "dispatch_routing", t_req_start, t_req_end)
    perf_metrics.add_stage_ns(req_id, "end_to_end", t_req_start, t_req_end)
